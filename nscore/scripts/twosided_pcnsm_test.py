...
from sequentialized_barnard_tests.savi import SaviTest, MirroredSaviTest
from sequentialized_barnard_tests.nsm import BernoulliNsmTest, PartialCreditNsmTest, MirroredPartialCreditNsmTest
...
if __name__ == "__main__":
    ...
    mirrored_savi_test = MirroredSaviTest(Hypothesis.P0LessThanP1, alpha=alpha)
    mirrored_pcnsm_test = MirroredPartialCreditNsmTest(Hypothesis.P0LessThanP1, alpha=alpha, c=c)
    ...
    if null_case or p1 <= 1.0:
        ...
        TIMES_TO_DECISION_MIRROR_SAVI = np.zeros(N_TRIALS)
        TIMES_TO_DECISION_MIRROR_PC_NSM = np.zeros(N_TRIALS)
        ...
        for kk in tqdm(range(N_TRIALS)):
            ...
            mirrored_savi_result = mirrored_savi_test.run_on_sequence(data0, data1)
            if mirrored_savi_result.decision == Decision.AcceptNull:
                try:
                    TIMES_TO_DECISION_MIRROR_SAVI[kk] = mirrored_savi_result.info["result_for_null"].info["Time"]
                except:
                    breakpoint()
            else:
                TIMES_TO_DECISION_MIRROR_SAVI[kk] = N + 1

            mirrored_pcnsm_result = mirrored_pcnsm_test.run_on_sequence(data_pc_0, data_pc_1)
            if mirrored_pcnsm_result.decision == Decision.AcceptNull:
                try:
                    TIMES_TO_DECISION_MIRROR_PC_NSM[kk] = mirrored_pcnsm_result.info["result_for_null"].info["Time"]
                except:
                    breakpoint()
            else:
                TIMES_TO_DECISION_MIRROR_PC_NSM[kk] = N + 1
        ...
        tmp_ttd_mirror_savi = np.sort(TIMES_TO_DECISION_MIRROR_SAVI)
        tmp_ttd_mirror_pc_nsm = np.sort(TIMES_TO_DECISION_MIRROR_PC_NSM)
        ...
        RESULTS[1, 3] = np.mean(TIMES_TO_DECISION_MIRROR_SAVI)
        RESULTS[1, 4] = np.mean(TIMES_TO_DECISION_MIRROR_PC_NSM)
        ...
        tmp_mirror_savi = np.argwhere(TIMES_TO_DECISION_MIRROR_SAVI < N + 0.5)
        tmp_mirror_pc_nsm = np.argwhere(TIMES_TO_DECISION_MIRROR_PC_NSM < N + 0.5)

        RESULTS[2, 0] = tmp_savi.shape[0] / N_TRIALS
        RESULTS[2, 1] = tmp_nsm.shape[0] / N_TRIALS
        RESULTS[2, 2] = tmp_pc_nsm.shape[0] / N_TRIALS
        RESULTS[2, 3] = tmp_mirror_savi.shape[0] / N_TRIALS
        RESULTS[2, 4] = tmp_mirror_pc_nsm.shape[0] / N_TRIALS
        ...
        print("Mirror SAVI: ", RESULTS[1, 3])
        print("Mirror PC_NSM: ", RESULTS[1, 4])
        print()
        print("Power (avg): ")
        print("SAVI: ", RESULTS[2, 0])
        print("NSM: ", RESULTS[2, 1])
        print("PC_NSM: ", RESULTS[2, 2])
        print("Mirror SAVI: ", RESULTS[2, 3])
        print("Mirror PC_NSM: ", RESULTS[2, 4])
