...
if __name__ == "__main__":
    ...
    np.save(f"data/BINARY/TTD_BINARY_NSM_alpha_{alpha}.npy", TTD_BINARY_NSM)
    np.save(f"data/BINARY/TTD_PC_NSM_alpha_{alpha}.npy", TTD_PC_NSM)
    np.save(f"data/BINARY/TTD_CONT_NSM_alpha_{alpha}.npy", TTD_CONT_NSM)
    np.save(f"data/BINARY/TTD_WSR_alpha_{alpha}.npy", TTD_WSR)
    np.save(f"data/BINARY/TTD_SAVI_alpha_{alpha}.npy", TTD_SAVI)
    np.save(f"data/BINARY/TTD_STEP_alpha_{alpha}.npy", TTD_STEP)

    np.save(
        f"data/BINARY/POWER_BINARY_NSM_alpha_{alpha}.npy", POWER_BINARY_NSM
    )
    np.save(
        f"data/BINARY/POWER_PC_NSM_alpha_{alpha}.npy", POWER_PC_NSM
    )
    np.save(
        f"data/BINARY/POWER_CONT_NSM_alpha_{alpha}.npy", POWER_CONT_NSM
    )
    np.save(
        f"data/BINARY/POWER_WSR_alpha_{alpha}.npy", POWER_WSR
    )
    np.save(
        f"data/BINARY/POWER_SAVI_alpha_{alpha}.npy", POWER_SAVI
    )
    np.save(
        f"data/BINARY/POWER_STEP_alpha_{alpha}.npy", POWER_STEP
    )
