...
if __name__ == "__main__":
    ...
    first_df = dict_of_dfs[df_keys[0]]
    ...
    # print()
    n_resamples = 1
    RESULTS_PART_1 = np.zeros((n_tasks, 7, 2, n_resamples))
    for ii in range(n_tasks):
        critical_idx = 2*ii

        # Success scores
        success_pi_0_base = np.multiply(np.array(first_df[df_columns[2]][critical_idx]), 1)
        success_pi_1_base = np.multiply(np.array(first_df[df_columns[2]][critical_idx + 1]), 1)
        ...
        # Progress scores
        progress_pi_0_base = np.multiply(np.array(first_df[df_columns[3]][critical_idx]), 1)
        progress_pi_1_base = np.multiply(np.array(first_df[df_columns[3]][critical_idx + 1]), 1)
        ...
        for jj in range(n_resamples):
            ...
            binary_step_test = MirroredStepTest(alternative=Hypothesis.P0LessThanP1, n_max=200, alpha=alpha)
            binary_wsr_test = WsrComparisonTest(alternative=Hypothesis.P0LessThanP1, alpha=alpha)

            # Progress metric results
            nsm_result = nsm_test.run_on_sequence(idx_list_0[idx_shuffled_progress].astype(int), idx_list_1[idx_shuffled_progress].astype(int))
            if nsm_result.decision == Decision.AcceptAlternative:
                ...
                RESULTS_PART_1[ii, 0, 0, jj] = 1
                RESULTS_PART_1[ii, 0, 1, jj] = nsm_time_of_decision
            ...
            nonparametric_nsm_result = nonparametric_nsm_test.run_on_sequence(progress_pi_0[idx_shuffled_progress], progress_pi_1[idx_shuffled_progress])
            if nonparametric_nsm_result.decision == Decision.AcceptAlternative:
                ...
                RESULTS_PART_1[ii, 1, 0, jj] = 1
                RESULTS_PART_1[ii, 1, 1, jj] = nonparametric_nsm_time_of_decision
            ...
            wsr_result = wsr_test.run_on_sequence(progress_pi_0[idx_shuffled_progress], progress_pi_1[idx_shuffled_progress])
            if wsr_result.decision == Decision.AcceptAlternative:
                wsr_time_of_decision = wsr_result.info["Time"]
                RESULTS_PART_1[ii, 2, 0, jj] = 1
                RESULTS_PART_1[ii, 2, 1, jj] = wsr_time_of_decision
            ...
            binary_step_result = binary_step_test.run_on_sequence(success_pi_0[idx_shuffled_success], success_pi_1[idx_shuffled_success])
            if binary_step_result.decision == Decision.AcceptAlternative:
                binary_step_time_of_decision = binary_step_result.info["Time"]
                RESULTS_PART_1[ii, 3, 0, jj] = 1
                RESULTS_PART_1[ii, 3, 1, jj] = binary_step_time_of_decision
            ...
            binary_nsm_result = binary_nsm_test.run_on_sequence(success_pi_0[idx_shuffled_success], success_pi_1[idx_shuffled_success])
            if binary_nsm_result.decision == Decision.AcceptAlternative:
                ...
                RESULTS_PART_1[ii, 4, 0, jj] = 1
                RESULTS_PART_1[ii, 4, 1, jj] = binary_nsm_time_of_decision
            ...
            binary_savi_result = binary_savi_test.run_on_sequence(success_pi_0[idx_shuffled_success], success_pi_1[idx_shuffled_success])
            if binary_savi_result.decision == Decision.AcceptAlternative:
                ...
                RESULTS_PART_1[ii, 5, 0, jj] = 1
                RESULTS_PART_1[ii, 5, 1, jj] = binary_savi_time_of_decision
            ...
            binary_wsr_result = binary_wsr_test.run_on_sequence(success_pi_0[idx_shuffled_success], success_pi_1[idx_shuffled_success])
            if binary_wsr_result.decision == Decision.AcceptAlternative:
                binary_wsr_time_of_decision = binary_wsr_result.info["Time"]
                RESULTS_PART_1[ii, 6, 0, jj] = 1
                RESULTS_PART_1[ii, 6, 1, jj] = binary_wsr_time_of_decision
        ...
        print("Task: ")
        print(first_df[df_columns[1]][critical_idx])
        print()
        # print("Mean progress amounts: ")
        # print("Policy 0: ", np.mean(progress_pi_0))
        # print("Policy 1: ", np.mean(progress_pi_1))
    ...
    np.save("data/LBM/LBM_RESULTS_PART_1.npy", RESULTS_PART_1)