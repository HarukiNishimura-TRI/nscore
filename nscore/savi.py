...
class PartialCreditSaviTest(SequentialTestBase):
    ...
    def __init__(
        ...
        # Store info about partial credit outcomes
        self.c = c
    ...
    def step(
        ...
        if (
            self.alternative == Hypothesis.P0MoreThanP1
            and self._estimated_mu_0 <= self._estimated_mu_1
        ) or (
            self.alternative == Hypothesis.P0LessThanP1
            and self._estimated_mu_0 >= self._estimated_mu_1
        ):
            ...
            x = cp.Variable(2*self.K)
            obj = cp.Minimize(cp.sum(cp.kl_div(x, ob_mat_1)))
            constraints = []
            c_mat_1 = np.eye(2*self.K)
            c_mat_2 = np.ones(2*self.K)
            c_mat_2[:self.K] *= 0.
            c_mat_3 = np.ones(2*self.K)
            c_mat_3[self.K:] *= 0. 
            c_mat_4 = np.zeros(2*self.K)
            if self.alternative is Hypothesis.P0LessThanP1:
                # NULL is P0 MORE than P1
                c_mat_4[:self.K] = copy.deepcopy(self.c)
                c_mat_4[self.K:] = -copy.deepcopy(self.c) 
            else:
                c_mat_4[:self.K] = -copy.deepcopy(self.c)
                c_mat_4[self.K:] = copy.deepcopy(self.c)

            constraints.append(c_mat_1 @ x >= 0.)
            constraints.append(c_mat_2 @ x <= 1.)
            constraints.append(c_mat_3 @ x <= 1.)
            constraints.append(c_mat_4.T @ x >= 0.)
        ...
        vec_datum_1 = np.zeros(self.K)
        vec_datum_1[datum_1] += 1.
        
        increment = (
            multinomial.logpmf(vec_datum_0, n=1, p=self._estimated_p_0)
            + multinomial.logpmf(vec_datum_1, n=1, p=self._estimated_p_1)
